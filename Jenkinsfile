pipeline {
    ...
    stages {
        stage('Build') { 
            steps {
		sh "docker run --name zapcontainer -u root -v $PWD:/zap/wrk -t owasp/zap2docker-weekly zap-baseline.py -t http://0.0.0.0:8080 -g gen.conf -a -j -r owaspzap.html || true"
