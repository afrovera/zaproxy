pipeline {
    ...
    stages {
		...
		sh "docker run --name owasp_zapcontainer -u root -v $PWD:/zap/wrk -t owasp/zap2docker-weekly zap-baseline.py -t http://172.17.0.3:8080 -g gen.conf -a -j -r owasp_zap_report.html || true"
